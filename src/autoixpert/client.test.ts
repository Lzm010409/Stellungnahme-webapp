...
describe('holeGutachten', () => {
  it('holt über den Pfad und schickt den Bearer-Token', async () => {
    ...
    const client = new AutoixpertClient({ token: 'geheim', basisUrl: BASIS, hole })
  ...
  it('meldet einen abgelehnten Zugang verständlich', async () => {
    ...
    const client = new AutoixpertClient({ token: 'falsch', basisUrl: BASIS, hole })
  ...
  it('meldet das Anfragenlimit gesondert', async () => {
    ...
    const client = new AutoixpertClient({ token: 't', basisUrl: BASIS, hole })
